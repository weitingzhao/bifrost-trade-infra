...
| Wave 14E cleanup | ✅ **2026-08-24** | `_deprecated/` renamed back to `sepa/*.py`; `SEPA_USE_ANALYTICS` flag & 12 gate call sites removed; shim layer eliminated |
| Wave 14F cleanup | ✅ **2026-08-24** | Retired FE-zero SEPA online POST routes + engines; removed Trade max-pain/phase4 stubs; archived `rollout-tibm-*.sh`; dropped `massive-ws` workload aliases |
| Wave 14G-A orphan cleanup | ✅ **2026-08-24** | Delete `sepa_readiness` shim; retire readiness-refresh Cron/no-op; drop empty `bifrost_worker.data`; Console Flex copy `flex_ops.*` → `ops_jobs.*` |
...
| DB Hygiene Wave 6 | ✅ **2026-08-24** | `ops_audit_log` retired → platform `POST /api/v1/audit/append`; core `0.15.0`; DEV/STG/PROD DROP verified — [WAVE6_ROLLOUT.md](WAVE6_ROLLOUT.md) |
