...
`crictl --timeout=10m rmi --prune` 之后四个节点回收 **647 GB**：

| 节点 | 前 | 后 | 回收 | 镜像 |
|---|---|---|---|---|
| ubt-k3s-04（PG 副本） | 343 GB · 78% | 102 GB · 23% | **241 GB** | 593 → 43 |
| ubt-k3s-05 | 284 GB · 65% | 50 GB · 12% | **234 GB** | 645 → 43 |
| ubt-k3s-01（registry） | 287 GB · 65% | 183 GB · 42% | **104 GB** | 294 → 22 |
| ubt-k3s-06 | 85 GB · 20% | 17 GB · 4% | **68 GB** | 300 → 20 |

`ubt-k3s-02`（Postgres 主库，剩 142 GB）**没有 prune**，等一个安静窗口。
