...
On agent session stop, `.claude/hooks/on-session-stop.js` (Claude `Stop` hook) POSTs session metadata to platform-api (program id, phase, duration). Ensure `PLATFORM_API_URL` and `PLATFORM_OPERATOR_TOKEN` are configured locally. Cursor 侧等价物是 `.cursor/hooks/on-session-stop.js`.
