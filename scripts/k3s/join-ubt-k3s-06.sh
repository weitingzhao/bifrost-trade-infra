#!/usr/bin/env bash
# Join ubt-k3s-06 (192.168.10.79) as K3s agent.
#
# This host is the reinstalled former bare-metal PG box (.80 decommissioned
# 2026-06-29) — now a plain general-pool K3s agent.
#
# Run from MacBook (bifrost-trade-infra root):
#   make k3s-join-ubt-k3s-06
#
...
AGENT_HOST="${AGENT_HOST:-vision@192.168.10.79}"
K3S_URL="${K3S_URL:-https://192.168.10.73:6443}"
K3S_NODE_IP="${K3S_NODE_IP:-192.168.10.79}"
K3S_NODE_NAME="${K3S_NODE_NAME:-ubt-k3s-06}"
KUBECONFIG="${KUBECONFIG:-${HOME}/.kube/bifrost-k3s.yaml}"
SSH_OPTS=(-o ConnectTimeout=15 -o ServerAliveInterval=30)

K3S_NODE_LABELS="${K3S_NODE_LABELS:-bifrost.io/host-id=ubt-k3s-06,bifrost.io/workload-pool=general}"

REMOTE_TOKEN_PATH="/home/vision/.bifrost-k3s-node-token"

echo "== ubt-k3s-06 K3s agent join =="
...
for pair in "${PAIRS[@]}"; do
  kubectl --kubeconfig "${KUBECONFIG}" label node "${K3S_NODE_NAME}" "${pair}" --overwrite || true
done
