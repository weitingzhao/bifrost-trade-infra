...
k3s-install-observability-remote:
	@chmod +x scripts/k3s/install-kube-prometheus-stack.sh
	KUBECONFIG=$(KUBECONFIG) ./scripts/k3s/install-kube-prometheus-stack.sh
...
# Point Gitea push webhooks at the CI EventListener. Without this the CI layer
# is installed but inert — pipelines exist and nothing ever fires.
k3s-install-ci-webhooks:
	@chmod +x scripts/k3s/install-ci-webhooks.sh
	KUBECONFIG=$(KUBECONFIG) ./scripts/k3s/install-ci-webhooks.sh
...
k3s-deliver-platform:
	@chmod +x scripts/k3s/run-deliver-platform.sh scripts/k3s/bootstrap-gitea-mirrors.sh scripts/sync_platform_k8s_config.sh scripts/k3s/apply-cicd-platform-pipeline.sh
	KUBECONFIG=$(KUBECONFIG) ./scripts/k3s/run-deliver-platform.sh

# Tekton bifrost-deliver-platform + platform-stg overlay (grafana env). No PipelineRun.
k3s-apply-cicd-platform-pipeline:
	@chmod +x scripts/k3s/apply-cicd-platform-pipeline.sh scripts/sync_platform_k8s_config.sh
	KUBECONFIG=$(KUBECONFIG) ./scripts/k3s/apply-cicd-platform-pipeline.sh
